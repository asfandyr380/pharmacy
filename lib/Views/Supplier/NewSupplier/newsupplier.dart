...
import 'package:medical_store/Views/Supplier/NewSupplier/newSupplierViewModel.dart';
...
class NewSupplierView extends StatefulWidget {
  final UserModel? supplier;
  const NewSupplierView({this.supplier});

  @override
  _NewSupplierViewState createState() => _NewSupplierViewState();
}

class _NewSupplierViewState extends State<NewSupplierView> {
  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<NewSupplierViewModel>.reactive(
      viewModelBuilder: () => NewSupplierViewModel(),
      onModelReady: (model) => model.init(widget.supplier),
      builder: (ctx, model, child) => SingleChildScrollView(
        child: Column(
          children: [
            PageIndicationText(text: 'New Supplier'),
            ListingBox(
              boxContent: ListingBoxContent(
                contentWidget: NewMedicineContent(
                  ...
                  onSubmit: () => widget.supplier != null
                      ? model.updateSupplier(widget.supplier)
                      : model.createSupplier(),
                ),
                headerTxt: 'New Supplier',
                isSearchable: true,
              ),
            ),
            SizedBox(height: 20),
